...
### 49. 多量体の取りこぼしは `repeat_fraction` ではなく「周期 = L/k」仮定が原因
**症状 (2026-09-03, plasmid DB を現 results へ揃え直したときに判明)**: 同一クラスタの
プラスミドが検体によって単量体だったり多量体だったりする。**同じ 3,861 bp の単量体を、
ある検体では 1×、別の検体では 4.85× で DB に持っていた** (クラスタ AD996)。
`collapse_concatemers` は動いてはいて、同じ検体の別 contig では発火している
(16656 は `contig_5` 12,388→6,194 を畳みながら 17,327 bp の AA106 を取りこぼし)。

**真因は閾値ではない。** 旧実装は「全長 L が判っているので候補周期は L/k に限られる」と
仮定していたが、**Flye が出す多量体は k 個ちょうどではない**:

| 検体 | 構造 | L/P |
|---|---|---|
| 20914 contig_1 | 部分コピー 4,110 + **4,502** × 4 | 4.91 |
| 16656 contig_15 | **3,861** × 4 + 部分 3,284 | 4.85 |
| 16656 contig_16 | **5,895** × 2 + 部分 5,537 | 2.94 |
| 20926 contig_2 | **21,326** + 20,160 (一方に 1,166 bp 欠失) | 1.95 |
| 22188 contig_3 | **21,326** + 19,696 (同 2,462 bp 欠失) | 1.92 |

L/k は真の周期から数百 bp 外れ、許容幅 (0.02) を越えて **support が 0 になり候補が
1 つも立たない**。つまり `repeat_fraction 0.95` の判定に**到達すらしていなかった**。
gap 分布の最頻値を実測すると、いずれも **DB に登録済みの単量体長と完全一致**する。
**「閾値が厳しい」という仮説で閾値だけ動かしても直らない** — 測るべきは閾値の値ではなく
分布そのものだった ([[feedback_verify_against_production_config]] と同型の教訓)。

#### 閾値を下げるだけでは足りない — 検証を blastn に移した理由
コピー間の相違が 2 種類あり、**どちらも固定周期の k-mer ペアリングでは見えない**:
- **登録ずれ (indel)**: 20926 は 2 コピーの長さが 1,166 bp 違う。欠失より後ろは
  delta が 21,326 ではなく 20,160 になるので、単一周期でのペアリングはそこで全滅する
  (実測 `repeat_fraction` 0.66)。
- **塩基の乖離**: 20914 のコピー間同一性は blastn で **89〜94%**。21-mer の生存率は
  `0.9^21 ≈ 0.11` しかなく、**k を 13 まで下げても回復しなかった** (実測)。

blastn は両方を吸収する。py39 に既存で DB 構築も不要 (`-subject` 方式は
`screen_known_vectors.py` / `compare_plasmid_structure.py` と同じ)。実測 0.1〜1 秒/contig。
判定は「**contig 全体が単量体で説明できるか**」+ **未説明の連続区間の上限**
(割合だけだと、散在する小さな未説明とまとまったユニーク領域を区別できない)。
blastn が無ければ k-mer に落ちるが**緩めない** — 通らないことは「多量体でない」ことを
意味しないので、見送った事実を記録する (#28 と同じ趣旨)。

#### 実装で確定したこと (どれも実測で 1 回ずつ間違えた)
- **周期は 3 候補を出して検証で選ぶ**: 最頻値 / 整数コピー数に丸めた L/k / 許容窓内の
  中央値。近接した山が許容幅の中で融合すると argmax が数百 bp ずれ (24268 contig_6:
  真の周期 50,090 に対し最頻値 49,242 → 730 bp が説明できず却下)、逆に丸めた値だけを
  採ると**単量体が数十〜百 bp 削れる** (20914 で 4,502→4,423、16656 contig_16 で
  5,895→5,776)。登録される単量体長が変わるので実害がある。
- **「いちばんよく説明できた候補」を全体から選ぶのは誤り。** コピー間に乖離があると
  周期が長いほど説明率が上がるので、必ず浅い縮約に倒れる (20914 が 5×4,502 ではなく
  2×9,001 になった)。**コピー数の深い順にグループを見て、最初に通ったグループで確定する。**
- **群内の順位付けでは support を周期より先に見る。** 説明率は「周期が真の周期以上なら
  必ず 1.0」なので**それだけでは周期を特定できない** (真の周期 P と L-P の間はどれも
  全体を説明する)。実測 TAS157_ONT contig_1 は真の周期 3,306 (support 100%) と
  L/2=3,536 (support 0) がどちらも説明率 1.0 / 未説明 0 bp で並び、周期の大きい方を
  優先していたため**単量体でない 3,536 を採っていた**。
- **保持する単量体は先頭コピーとは限らない。** 20914 は**先頭コピーだけが異型**で、
  独立に組んだ同一分離株 (20914_BSI, 4,472 bp) はコピー 2〜5 と 95〜99% 一致、
  先頭コピーとは 1,608 bp が 83.0% しか一致しない。旧実装の `seq[:period]` は
  **少数派の異型を代表配列として登録していた**。全コピー窓を試し、残りをいちばん
  よく説明する窓を採る。
- **反復単位が `min_monomer_len` を割る contig は畳まない。** 実測 17559_BSI contig_38
  (4,428 bp) と kojima/17559_BSI contig_10 (4,108 bp) は **約 171 bp の単位が並んだ
  サテライト状の配列**で、1,024〜2,214 bp の**どの周期でも説明率 1.0 / 未説明 0 bp**
  になる。説明率では周期を選べないので、どれを採っても恣意的な倍数を単量体として
  登録することになる。単量体自身の周期性を調べて、下限を割るなら理由を残して見送る
  (**旧実装は contig_10 を誤って畳んでいた**)。

#### 誤縮約の全数確認 (配列を書き換える処理なので必須)
`workflow/scripts/audit_concatemers.py` (読むだけ) で NAS 全 **534 アセンブリ**を走査。
**新規縮約 28 contig / 25 検体、誤縮約 0、退行 0** (30 秒)。
**28 件中 14 件は別検体の DB 登録単量体長と完全一致** (AA667=3,306 が 6 検体、
AA411=21,326 が 3 検体、AD996=3,861 が 3 検体、AA013=4,502 が 2 検体 …) =
独立に組んだアセンブリによる裏づけ。**閾値を変えたら必ずこれを通すこと。**
28 件のうち **11 件はモジュール導入前 (2026-07-13〜16) のアセンブリ**で、そもそも
collapse が走っていない分。取りこぼしは残り 17 件。

#### 既存検体は「再解析」で直す — 後追い縮約ツールは意図的に作っていない
`results/{sample}/assembly/long_read/contigs.fasta` を後から書き換えると、AMRFinder /
MOB-suite / Bakta / PlasAnn / integron / molecule_classification の座標が全部ずれる
(#19 / #22)。とくに `compare_plasmid_structure.collect_features` は
**「プラスミド FASTA は contig の抜き出しとバイト一致する」**ことを前提に results 側の
座標を DB FASTA の軸へ載せているので、**DB の FASTA だけ単量体に畳むとコピー 2 以降の
遺伝子が単量体の外に描かれる**。DB 側だけのパッチも不可。
是正は該当 25 検体の**再解析**で、アセンブリ・アノテーション・DB 登録を揃えるのが
唯一整合する経路 (対象一覧は `audit_concatemers.py --summary`)。
**再解析すると #30 のとおり小型プラスミドの回収差も同時に変わる**ので、
「多量体の是正だけ」にはならない点は承知して実施すること。
**該当ファイル**: `workflow/scripts/concatemer_core.py` (新規・判定の単一の真実源),
`workflow/scripts/audit_concatemers.py` (新規・全数点検),
`workflow/scripts/collapse_concatemers.py` (core を呼ぶだけに),
...
#### 切替に失敗した (2026-09-07) — 参照ゲノムは名前ではなく配列で確かめる
20,704 株の新 DB へ切り替えたところ、**2 種・最大 49 検体の菌種同定が劣化**した。
差し戻して作り直した。原因は独立した 2 つで、**どちらも「1 種 1 株」に絞る指定が
引き金**になっている。

1. **上流が差し替えた誤同定株を掴んだ。** *Enterobacter kobei* は上流
   `references.db` に 3 株しか無く全て `suspected-type` (旧 DB の DSM 13645 は
   収録されなくなった)。同順位のタイブレークが**アクセッション文字列順**だったため
   `GCA_001940295.1` (JCM 8580, Scaffold) を中身も見ずに選んだが、これは
   **旧参照 `GCA_001729765.1` (DSM 13645) との identity が 0.8384** しかない
   別物だった (同一菌種なら 0.98 前後)。臨床分離株 19424 / 19429 は旧参照に
   ANI 98.80% / 98.67% で一致していたのに Unknown へ落ちた。
   **菌種名も株名 (どちらも "JCM 8580") もカテゴリも取り違えを示さない —
   配列を比べるしかない。**
2. **上流の亜種統合で、使われている参照が消えた。** *E. hormaechei* の亜種が
   1 つの `species_taxid` にまとめられたため、`--dedupe-by-species` が
   subsp. hoffmannii を落とした (優先度判定は `type` > `suspected-type` で正しい)。
   残った subsp. hormaechei との identity は **0.9477** で、実測 4 検体が
   0.958〜0.985 → 0.945〜0.951 と**菌種境界 0.95 を跨いだ**。対象は 47 検体。

**切替前の検証が本番の判定を再現していなかった (#38 の再発)。**
`compare_typestrain_dbs.py` は **mash screen の取得下限 0.9** で最上位の菌種名を
比べており、`merge_species_id` の**菌種境界 0.95** を適用していなかった。
そのため 19424 / 19429 (0.9247 / 0.9294) を「最上位が変化」に数え、
**「同定できなくなった 0 件」と報告して切替を通した**。
逆に 19397 は mash 最上位が *E. dykesii* に変わったが FastANI 96.81% で
*E. asburiae* のまま = **mash の最上位変化は最終判定の変化を意味しない**。
検証ツールは**両方の閾値を通した最終判定**を比べること。

**入れた対策**:
- タイブレークを **assembly_level 順** (完成ゲノム優先) にし、
  `--assembly-summary` が無ければ **dedupe を拒否する** (恣意的な文字列順に
  戻らせない)。実データで `GCA_018323985.1` (Complete Genome) が選ばれる
- **`--keep-from-csv`** — 現行 DB にしか無い株を引き継ぐ。上流の削除・亜種統合で
  実際に使われている参照が消えるのを防ぐ。実測で DSM 13645 と hoffmannii が
  両方復活し 22,819 株 (要ダウンロードは差分 2,045 株のみ)
- **`--check-references`** — 入れ替わった参照どうしの mash 距離を測り
  0.95 未満で止める。これがあれば E. kobei は切替前に捕まった
- 比較の判定に **`--species-identity` (既定 0.95)** を適用

**作り直して切替に成功した (2026-09-07, `DFAST_typestrain_20260907` / 22,782 株)**。
`--keep-from-csv` (+2,078 株) と assembly_level 順のタイブレークで、
E. kobei DSM 13645 と E. hormaechei subsp. hoffmannii が戻った。検証は
`--check-references` が止める入れ替わり 0 件 (版上げ 36 件・再分類 1 件は別枠)、
検体比較 82 件が **同定できなくなった 0 件** (19424 / 19429 は Unknown →
*E. kobei* 0.9895 / 0.9893 = 旧 DB と同一の値、E. hormaechei 47 検体は全て一致)。
**引き継ぎの重複判定は版 (`.1` / `.2`) を除いて行うこと** — 版込みで比べると
同じアセンブリが二重に入り、同じゲノムを 2 回スケッチして
CSV 行数とスケッチ件数がずれる (実測 37 件。preflight が捕まえた)。
**preflight の期待値は CSV 行数ではなく「ゲノムが在る株数」**にすること —
上流から取り下げられた assembly は恒久的に落とせないので、完全一致を求めると
正常な DB でも永久に検証できない (実測 2 株)。

**影響範囲の測り方**: NAS 全検体の `{sample}_summary.json` から菌種を集めると
19 種しか無かった (1,102 検体)。**参照が変わったのはこの 2 種だけで、17 種は同一。**
菌種名は subsp. 表記が揺れるので**属+種で照合すること** (完全一致だと
*Salmonella enterica* 354 検体が「どちらにも無い」に落ちる)。
